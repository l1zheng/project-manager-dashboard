...
Outlook automation is isolated behind a `MailDraftAdapter`. Production code receives only a subject and a server-generated HTML fragment. It does not accept recipients, attachments, arbitrary script paths, commands, or raw client-supplied HTML.
...
Subject and HTML never appear in command-line arguments. Node writes a bounded UTF-8 JSON request into a unique user-temporary directory and passes only that file path as one argument. The script reads and validates the JSON as data. Node deletes the temporary directory in `finally`, limits captured output, and terminates a bridge invocation that exceeds the configured timeout.
...
The canonical report model remains the sole source of data. The Outlook renderer produces a fragment with table layout, inline styles, conservative fonts, no scripts, no external stylesheets, no remote images, and escaped text. URL values are text in the first release rather than executable links.
...
- Unit-test Outlook fragment escaping, plain-text output, subject normalization, request-size limits, process arguments, timeouts, exit-code mapping, and temporary-file cleanup.
...
- On the target Windows machine, verify classic Outlook detection, signature retention, Chinese and long-text rendering, multiple independent tables, status highlighting, and visible compose-window behavior.
